...
//
// A Zappier zap is configured to watch for SE Activities that appear in the Upload tab, and push them up 
// into Salesforce
// 
...
function import_gcal_(calName, startDate, endDate, cursor){
  ...
  for (var i=0;i<events.length;i++) {
    ...
    */
    ...
    let range=cursor.sheet.getRange(cursor.row,1,1,11); // 11 must match details
...
function expand_se_events() {
  ...
  //
  // Load All Customer Info (Better include our Region - The salesforce reports should be )
  //
...
function process_account_emails_(accountInfo, numberOfRows, accountType, accountName, accountId, emailFieldNumber, altEmailFieldNumber, emailToAccountMap) {
  ...
  for (var j = 0 ; j < numberOfRows; j++) {
    ...
    if (altDomainString) {
      let moreDomains = emailDomainString.split(',');
...
function build_se_events() {
  ...
  for (j = 0 ; j < lastRow - 1; j++) {
    ...
    // REMOVEME commented this out for Guy
    /*
    ...
    */
    ...
    let parms = SpreadsheetApp.getActive().getSheetByName(RUN_PARMS);
    let overrideRange = parms.getRange(4,7,16,2); // Hardcoded to format in RUN_PARMS
    ...
    for (var row in overrides) {
      if (overrides[row][0]) {      
        ...
        let regex =  new RegExp(overrides[row][1]);
        if (regex.test(inviteInfo[j][SUBJECT])) {
